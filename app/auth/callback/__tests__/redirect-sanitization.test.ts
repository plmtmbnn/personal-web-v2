...
describe("Auth Callback Redirect Sanitization", () => {
	describe("Valid admin routes", () => {
		...
		it("should allow /admin with subpaths", () => {
			...
			expect(sanitizeRedirectPath("/admin/blog/editor")).toBe(
				"/admin/blog/editor",
			);
			expect(sanitizeRedirectPath("/admin/blog/editor/123")).toBe(
				"/admin/blog/editor/123",
			);
		});
		...
		it("should allow utility admin pages", () => {
			expect(sanitizeRedirectPath("/utils/stock-explorer/admin")).toBe(
				"/utils/stock-explorer/admin",
			);
			expect(sanitizeRedirectPath("/utils/sql-formatter/admin")).toBe(
				"/utils/sql-formatter/admin",
			);
		});
	});
