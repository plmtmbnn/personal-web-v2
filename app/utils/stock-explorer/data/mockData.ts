...
export const getMockSector = (stockCode: string): Sector => {
	...
	if (["BBCA", "BBRI", "BMRI", "BBNI", "ARTO", "BRIS"].includes(stockCode)) return "Banking";
	if (["GOTO", "BUKA", "WIFI", "EMTK"].includes(stockCode)) return "Technology";
	if (["ADRO", "PTBA", "ITMG", "PGAS", "MEDC"].includes(stockCode)) return "Energy";
	if (["KLBF", "MIKA", "SILO", "HEAL"].includes(stockCode)) return "Healthcare";
	if (["TLKM", "EXCL", "ISAT", "JSMR", "WIKA"].includes(stockCode)) return "Infrastructure";
	if (["UNVR", "ICBP", "INDF", "MYOR", "AMRT"].includes(stockCode)) return "Consumer";
	if (["BSDE", "CTRA", "SMRA", "PWON", "ASRI"].includes(stockCode)) return "Property";
	if (["ASII", "UNTR", "GIAA", "BIRD"].includes(stockCode)) return "Transportation";
	if (["ANTM", "INCO", "MDKA", "TINS", "BRPT"].includes(stockCode)) return "Basic Materials";

	return sectors[hashString(stockCode) % sectors.length];
};

export const getMockFundamentals = (stockCode: string, closePrice: number): FundamentalData => {
	...
	const multiplier = ["BBCA", "BBRI", "BMRI", "TLKM", "ASII"].includes(stockCode) ? 100 : ((hash % 10) + 1);
