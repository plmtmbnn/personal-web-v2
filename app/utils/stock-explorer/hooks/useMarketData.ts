...
export function useMarketData(rawData: IDXStock[]) {
	return useMemo(() => {
		...
		const processed: ProcessedStock[] = rawData.map((stock) => {
			...
			const ChangePct = stock.Previous > 0 ? (stock.Change / stock.Previous) * 100 : 0;
			...
			let priceScore = Math.min(Math.max((ChangePct + 5) * 10, 0), 100); // Scale -5% to +5% into 0-100
			let volScore = stock.Volume > 100000000 ? 100 : (stock.Volume / 100000000) * 100;
			let foreignScore = ForeignNet > 0 ? 50 + Math.min((ForeignNet / 50000000000) * 50, 50) : 50 - Math.min((Math.abs(ForeignNet) / 50000000000) * 50, 50);
			let liqScore = stock.Frequency > 10000 ? 100 : (stock.Frequency / 10000) * 100;
			let volaScore = 100 - (Math.abs(stock.High - stock.Low) / (stock.Previous || 1)) * 1000; // Lower volatility = higher score (or depends on strategy, but we assume stability is good)
			...
			if (CompositeScore > 85 && ChangePct > 2 && ForeignNet > 1000000000) Opportunity = "🔥 Strong Buy";
			else if (ChangePct > 4 && stock.Volume > 50000000) Opportunity = "🚀 Momentum";
			else if (stock.High === stock.Close && ChangePct > 2) Opportunity = "📈 Breakout";
			else if (ForeignNet > 5000000000 && ChangePct < 1) Opportunity = "🌍 Foreign Accumulation";
			else if (CompositeScore < 40 && ChangePct < -2) Opportunity = "❌ Weak Trend";
			else if (stock.Value > 100000000000) Opportunity = "🏦 Blue Chip"; // Proxy for blue chip
		...
		let sentimentScore = 50 + ((adRatio - 1) * 10) + (avgReturn * 10) + (netForeign > 0 ? 10 : -10);
