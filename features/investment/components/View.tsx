...
export default function InvestmentPage() {
	...
	const LoadingStrategicBanner = () => (
		<motion.div
			initial={{ opacity: 0, y: 10 }}
			animate={{ opacity: 1, y: 0 }}
			className={`p-6 sm:p-8 rounded-3xl border shadow-lg bg-slate-50 flex flex-col md:flex-row items-center justify-between gap-6`}
		>
			<div className="flex items-start gap-4 w-full">
				<div className="w-12 h-12 rounded-2xl bg-slate-200 flex-shrink-0" />
				<div className="space-y-2 flex-1">
					<div className="flex items-center gap-2 flex-wrap">
						<div className="px-3 py-1 rounded-full bg-slate-200 animate-pulse w-24 h-5" />
						<div className="text-xs font-black bg-slate-200 animate-pulse w-16 h-5" />
					</div>
					<div className="text-sm font-black h-5 bg-slate-200 animate-pulse rounded w-48" />
					<div className="text-xs text-slate-500 leading-relaxed max-w-3xl space-y-1">
						<div className="h-4 bg-slate-200 animate-pulse w-full rounded" />
						<div className="h-4 bg-slate-200 animate-pulse w-5/6 rounded mt-1" />
					</div>
				</div>
			</div>
			<div className="w-full md:w-auto">
				<div className="px-5 py-3 bg-slate-200 animate-pulse rounded-xl text-xs font-black uppercase tracking-wider w-full" />
			</div>
		</motion.div>
	);
