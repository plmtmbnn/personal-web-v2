				...
				<div className="flex-1 min-w-0">
					...
					<div className="flex flex-wrap gap-2.5 mb-10">
						{staleTasks.slice(0, 3).map((task) => (
							<div
								key={task.id}
								className="flex items-center gap-3 px-4 py-2 bg-white border border-slate-200 rounded-xl shadow-sm group hover:border-amber-300 transition-colors"
							>
								...
								<span className="truncate max-w-[180px] font-bold text-slate-800 text-xs">
									{task.title}
								</span>
							</div>
						))}
