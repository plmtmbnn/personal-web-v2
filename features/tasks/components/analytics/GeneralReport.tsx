...
export default function GeneralReport({ tasks = [] }: GeneralReportProps) {
	...
	const reliabilityMetrics = useMemo(() => {
		if (!tasks || tasks.length === 0)
			return { punctualityRate: 0, executionScore: 0, avgLeadTime: 0 };
			...
			<AnimatePresence>
				{isExpanded && (
					<motion.div
						...
						className="overflow-hidden space-y-10 px-4 pb-10"
					>
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
							...
							<StatCard
								...
								value={`${reliabilityMetrics.executionScore}/100`}
								icon={Zap}
							...
							<StatCard
								title="Lead Time"
								value={`${reliabilityMetrics.avgLeadTime}d`}
								icon={Clock}
