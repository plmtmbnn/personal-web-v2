...
export default function StockExplorerView() {
	...
	return (
		<div className="min-h-screen bg-slate-50/80 bg-dot-pattern relative overflow-x-hidden text-slate-900 pb-32">
			{/* Top Nav Bar */}
			<div className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-slate-200/80 shadow-xs px-4 py-4 md:px-8 flex justify-between items-center">
				<div>
				...
				<div className="flex items-center gap-3 sm:gap-4">
					...
					<button
						onClick={fetchData}
						disabled={isLoading}
						className="flex items-center gap-2 px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold uppercase tracking-wider transition-all shadow-md active:scale-95 disabled:opacity-50 cursor-pointer"
					>
						{isLoading ? (
			...
			<main className="max-w-7xl mx-auto px-4 md:px-8 py-8 space-y-6">
				{error ? (
					<div className="bg-rose-50 border border-rose-200 rounded-3xl p-8 flex flex-col items-center justify-center text-center shadow-xs">
						<AlertCircle className="w-12 h-12 text-rose-600 mb-4" />
						<h3 className="text-lg font-extrabold text-rose-900 mb-2">
							Terminal Offline
						</h3>
					...
					<>
						...
						<div className="bg-white border border-slate-200/80 rounded-[2.5rem] shadow-xl shadow-slate-200/50 overflow-hidden">
							<button
								...
								className="w-full p-6 sm:p-8 flex justify-between items-center bg-white hover:bg-slate-50/80 transition-colors focus:outline-none cursor-pointer border-none text-left"
							>
								...
								<div className="flex items-center gap-4">
									<span className="hidden lg:inline-block text-[10px] font-bold uppercase tracking-wider text-indigo-700 bg-indigo-50 border border-indigo-100 px-3.5 py-1.5 rounded-full">
							...
							{isScorerExpanded && (
								<div className="p-6 sm:p-8 border-t border-slate-100 bg-slate-50/50 space-y-8">
									{/* Presets */}
									<div className="flex flex-wrap gap-2 items-center">
										...
										{[
											...
											<button
												...
												className="px-4 py-2 bg-white hover:bg-indigo-50 hover:text-indigo-600 border border-slate-200/80 text-slate-800 rounded-xl text-xs font-bold tracking-wider transition-all cursor-pointer shadow-2xs"
											>
												{preset.name}
											</button>
										))}
